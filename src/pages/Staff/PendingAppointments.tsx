...
const PendingAppointments = () => {
    ...
    try {
      ...
      if (res.success) {
        const mappedAppointments: Appointment[] = res.data.map((apt: any) => ({
          id: apt._id,
          status: apt.status,
          patientName: apt.customerId?.fullName || apt.patientUserId?.fullName || 'N/A',
  ...
  const handleReview = async (appointmentId: string, action: 'approve' | 'cancel') => {
    try {
        ...
        await appointmentApi.reviewAppointment(appointmentId, action, cancelReason);
        refetchAppointments();
    } catch (error) {
  ...
  return (
    <div className="bg-white rounded-lg shadow-sm border p-4 mt-6">
        <h2 className="text-xl font-bold text-gray-800 mb-4">Yêu cầu đặt lịch chờ xác nhận</h2>
        {error && (
  ...
  <DropdownItem key="approve" onPress={() => handleReview(appointment.id, 'approve')}>
    Duyệt
  </DropdownItem>
  <DropdownItem
    key="cancel"
