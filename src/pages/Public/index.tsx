      ...
      <div className="flex flex-col w-full min-h-screen">
        ...
        <div className="relative w-full h-[550px] bg-gradient-to-r from-gray-900 to-gray-800 mb-20">
          ...
          <div className="absolute -bottom-8 md:-bottom-12 lg:-bottom-14 left-1/2 transform -translate-x-1/2 z-30">
            <button
              className="group bg-white shadow-2xl rounded-2xl px-8 md:px-10 py-5 md:py-6 flex items-center gap-4 md:gap-5 transition-all duration-300 hover:shadow-[0_20px_50px_rgba(57,189,204,0.4)] hover:scale-105 hover:-translate-y-2 border-2 border-[#39BDCC]/20 hover:border-[#39BDCC] animate-breathing"
        ...
        <ServicesSection services={services} handleImageError={handleImageError} />
...
// Service Item Component with Scroll Animation
interface ServiceItemProps {
  ...
  index: number;
  handleImageError: (e: SyntheticEvent<HTMLImageElement, Event>, type: ImageErrorType) => void;
}

const ServiceItem = ({ service, index, handleImageError }: ServiceItemProps) => {
  ...
  return (
    <div
      ...
      } items-center gap-12 ${animationClass} ${isVisible ? 'visible' : ''}`}
    >
...
// Blog Card Component with Scroll Animation
...
const BlogCard = ({ blog, index, navigate, getCategoryLabel }: BlogCardProps) => {
  const { elementRef, isVisible } = useScrollAnimation({ threshold: 0.2 });

  return (
    <div
      ref={elementRef}
      className={`bg-white rounded-3xl shadow-lg overflow-hidden hover:-translate-y-2 hover:shadow-2xl transition-all duration-300 cursor-pointer scroll-reveal-up ${isVisible ? 'visible' : ''}`}
...
// Services Section Component with Scroll Animation
...
const ServicesSection = ({ services, handleImageError }: ServicesSectionProps) => {
  const { elementRef: sectionRef, isVisible: sectionVisible } = useScrollAnimation({ threshold: 0.1 });

  return (
    <div 
      ref={sectionRef}
      className={`max-w-7xl mx-auto pt-24 pb-20 px-6 scroll-reveal-up ${sectionVisible ? 'visible' : ''}`}
    >
...
// Blogs Section Component with Scroll Animation
...
const BlogsSection = ({ latestBlogs, isBlogLoading, blogError, navigate, getCategoryLabel }: BlogsSectionProps) => {
  const { elementRef: sectionRef, isVisible: sectionVisible } = useScrollAnimation({ threshold: 0.1 });

  return (
    <div 
      ref={sectionRef}
      className={`max-w-7xl mx-auto pb-24 px-6 scroll-reveal-up ${sectionVisible ? 'visible' : ''}`}
    >
